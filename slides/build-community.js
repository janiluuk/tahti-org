...
async function main() {
  ...
  // ─── Slide 1: title (artist-facing) ─────────────────────────────
  {
    ...
    s.addShape(pres.shapes.RECTANGLE, {
      ...
      w: 0.06,
      h: 0.5,
      fill: { color: COL.amber },
      line: { color: COL.amber },
    })
    s.addText('TAHTI', {
      x: 0.85,
      y: 0.5,
    ...
    s.addText(
      'One URL that always plays. Live when you\u2019re broadcasting, your archive when you\u2019re not. Listeners chat, download your tracks, and can support you directly with subscriptions \u2014 we take 0%, just a 2% fee for processing. Paid members broadcast in lossless. Free members get MP3 and one hour of live per week \u2014 a complete product, no friction.',
      {
        x: 0.7,
        y: 4.55,
        w: 11,
  ...
  // ─── Slide 2: the problem ───────────────────────────────────────
  {
    ...
    for (let i = 0; i < platforms.length; i++) {
      ...
      s.addShape(pres.shapes.RECTANGLE, {
        ...
        h: 1.4,
        fill: { color: COL.card },
        line: { color: COL.cardBorder, width: 1 },
        shadow: sh(0.3),
      })
      s.addShape(pres.shapes.RECTANGLE, {
        x,
        ...
        w: 0.08,
        h: 1.4,
        fill: { color: p.color, transparency: 30 },
        line: { color: p.color, transparency: 30 },
      })
      s.addText(p.name, {
        x: x + 0.25,
        y: y + 0.2,
    ...
    s.addText(
      [
        {
          text: 'And none of them are yours.',
          options: { color: COL.amber, bold: true, fontSize: 22, breakLine: true },
        },
        {
          text: 'You\u2019re renting attention from companies that change the rules whenever it suits them. Algorithm shifts. Pricing tiers. Acquisitions. Shutdowns.',
          options: { color: COL.textMuted, italic: true, fontSize: 14 },
        },
      ],
  ...
  // ─── Slide 2b: the structural problems with each ────────────────
  {
    const s = pres.addSlide()
    s.background = { color: COL.bg }

    s.addText('It\u2019s worse than just scattered.', {
      x: 0.7,
      y: 0.5,
      w: 12,
      h: 0.7,
    ...
    s.addText('Each platform has a structural problem you can\u2019t fix by trying harder.', {
      x: 0.7,
      y: 1.15,
      w: 12,
      h: 0.5,
    ...
    const critiques = [
      {
        x: 0.7,
        color: COL.rose,
        head: 'SoundCloud & Mixcloud',
        sub: 'Audio quality is capped at the listener tier, not you',
        body: 'You pay Pro. Your listeners still hear 128 kbps Opus (SoundCloud free) or 64 kbps AAC (Mixcloud free). Your master sounds like a YouTube rip to the people you made it for. Even when you pay, your audience suffers \u2014 unless they also pay.',
      },
      {
        x: 4.75,
        color: COL.amber,
        head: 'Spotify',
        sub: 'Overrun by AI, no required disclosure',
        body: 'Documented in Music Business Worldwide and Harper\u2019s: AI-generated tracks in curated playlists, \u201Cghost artist\u201D placeholder names, no labeling requirement. Your original work competes with infinite-supply autogenerated audio for the same algorithmic surfacing.',
      },
      {
        x: 8.8,
        color: COL.cyan,
        head: 'Bandcamp',
        sub: 'Pays fairly \u2014 but it\u2019s a store, not a station',
        body: '82-85% to artists is the best rate in commercial music. But Bandcamp is fundamentally a record store: no 24/7 channel, no live broadcasting, no real-time chat, no archive-fallback. Good at selling albums. Not what artists need for broadcasting.',
      },
    ]
    ...
    s.addShape(pres.shapes.RECTANGLE, {
      ...
      h: 0.5,
      fill: { color: COL.bg2 },
      line: { color: COL.mint, width: 1 },
    })
    s.addText(
      [
        {
          text: 'WHAT TAHTI DOES DIFFERENTLY:  ',
          options: { color: COL.mint, bold: true, charSpacing: 3 },
        },
        {
          text: 'Lossless audio for paid \u2014 your listeners hear what you made. No algorithm, no AI feed. Broadcasting + chat + downloads, not just storage.',
          options: { color: COL.textMuted, italic: true },
        },
      ],
      {
        x: 0.95,
  ...
  // ─── Slide 3: the answer — your channel ─────────────────────────
  {
    ...
    s.addText(
      [
        {
          text: 'NEVER A SILENT GAP.  ',
          options: { color: COL.amber, bold: true, charSpacing: 3 },
        },
        {
          text: 'When you stop, the archive starts within 10 seconds. Listeners stay tuned in.',
          options: { color: COL.textMuted, italic: true },
        },
      ],
      {
  ...
  // ─── Slide 4: broadcasting is easy ──────────────────────────────
  {
    ...
    const tools = [
      {
        name: 'OBS Studio',
        color: COL.cyan,
        note: 'RTMP \u00B7 video+audio',
        icon: FaBroadcastTower,
      },
      {
        name: 'Mixxx',
        color: COL.violet,
        note: 'Icecast \u00B7 open-source DJ',
        icon: FaCompactDisc,
      },
      {
        name: 'Traktor Pro',
        color: COL.mint,
        note: 'Icecast \u00B7 NI workflow',
        icon: FaWaveSquare,
      },
      {
        name: 'butt',
        color: COL.amber,
        note: 'Icecast \u00B7 minimal streamer',
        icon: FaMicrophone,
      },
    ...
    s.addText(
      [
        {
          text: 'Open your dashboard. Click "How to broadcast." Pick your tool. ',
          options: { color: COL.textMuted },
        },
        {
          text: 'Your credentials are pre-filled and ready to copy-paste. ',
          options: { color: COL.textLight, bold: true },
        },
        { text: 'Click ', options: { color: COL.textMuted } },
        { text: '"Test connection" ', options: { color: COL.cyan, italic: true, bold: true } },
        {
          text: 'to verify your setup with a 10-second probe before you go live for real. Stream keys are rotatable from the dashboard whenever you need.',
          options: { color: COL.textMuted },
        },
      ],
  ...
  // ─── Slide 5: live chat is the centerpiece ──────────────────────
  {
    ...
    s.addShape(pres.shapes.RECTANGLE, {
      ...
      w: 6.1,
      h: 0.7,
      fill: { color: COL.amber, transparency: 80 },
      line: { color: COL.amber, width: 1 },
    })
    s.addText(
      [
        ...
        {
          text: 'PINNED  \u00B7  YOUR ARTIST',
          options: { color: COL.amber, bold: true, fontSize: 9, charSpacing: 3, breakLine: true },
        },
        {
          text: 'Tonight 22:00 UTC \u2014 ambient set, three new originals',
          options: { color: COL.textLight, fontSize: 12, italic: true },
        },
      ],
    ...
    const msgs = [
      ...
      { who: 'midnight_dj', color: COL.violet, txt: 'what synth is this?' },
      {
        who: 'tallinn_owl',
        color: COL.mint,
        txt: 'tuned in from Tallinn \uD83C\uDDEA\uD83C\uDDEA',
      },
      { who: 'ghost', color: COL.rose, txt: 'first time here \u2014 this is amazing' },
      {
        who: 'you (artist)',
        color: COL.amber,
        txt: '@midnight_dj \u2014 Prophet 5, sent through tape delay',
      },
  ...
  // ─── Slide 6: your home page (profile + releases) ──────────────
  {
    ...
    s.addShape(pres.shapes.RECTANGLE, {
      ...
      h: 0.85,
      fill: { color: COL.bg2 },
      line: { color: COL.amber, width: 1.5 },
      shadow: sh(0.35),
    })
    s.addText(
      [
    ...
    s.addShape(pres.shapes.RECTANGLE, {
      ...
      h: 0.5,
      fill: { color: COL.bg2 },
      line: { color: COL.textDim, width: 1 },
    })
    s.addText(
      [
        {
          text: "WHAT'S NOT ON YOUR PROFILE:  ",
          options: { color: COL.textDim, bold: true, charSpacing: 3 },
        },
        {
          text: 'no follower count, no track-level comments, no algorithmic feed. This is a label page, not a social profile.',
          options: { color: COL.textMuted, italic: true },
        },
      ],
      {
  ...
  // ─── Slide 7: tools to spread the word (promo toolkit) ─────────
  {
    ...
    s.addText(
      'Embed, smart links, newsletter, social auto-post, analytics \u2014 the things every artist asks for.',
      {
        x: 0.7,
        y: 1.15,
        w: 12,
  ...
  // ─── Slide 8: distribution ──────────────────────────────────────
  {
    ...
    for (const ln of lanes) {
      ...
      s.addShape(pres.shapes.RECTANGLE, {
        ...
        h: 1.15,
        fill: { color: COL.bg2 },
        line: { color: COL.cardBorder, width: 0.5 },
      })
      s.addText(ln.cta, {
        x: ln.x + 0.4,
        y: 5.7,
  ...
  // ─── Slide 7: your archive, your storage ────────────────────────
  {
    ...
    s.addText(
      'No enforced storage limits. No \u201Cyou\u2019ve used 80% of your quota\u201D anxiety.',
      {
        x: 0.7,
        y: 1.15,
        w: 12,
    ...
    s.addText(
      [
        ...
        {
          text: 'We track storage and display it openly on the public dashboard. We never cut you off. Written into the bylaws \u2014 see for yourself.',
          options: { color: COL.textMuted, italic: true },
        },
      ],
      {
  ...
  // ─── Slide 8: how money reaches you (engagement units + fan-subs) ─
  {
    ...
    s.addText(
      'Direct from fans who support you. Plus a yearly grant from the org\u2019s surplus, weighted by engagement.',
      {
        x: 0.7,
        y: 1.15,
        w: 12,
    ...
    const flows = [
      {
        ...
        body: [
          {
            text: 'Fans subscribe directly to you for \u20AC1\u2013\u20AC100/month. ',
            options: { color: COL.textLight, bold: true },
          },
          {
            text: 'Money flows fan \u2192 Stripe \u2192 you, with a 2% operational fee covering processing, GDPR, and support. ',
            options: { color: COL.textMuted },
          },
          { text: 'Tahti ry takes zero. ', options: { color: COL.amber, bold: true } },
          {
            text: 'Subscribers get a badge, FLAC downloads, fan-only chat. You choose tiers and benefits.',
            options: { color: COL.textMuted },
          },
        ],
        fig: '\u20AC163k',
        figLabel: 'to artists, 3yr total',
      },
      {
        x: 6.85,
        color: COL.mint,
        ...
        body: [
          {
            text: 'After audit each year, 90% of org surplus flows to artists as grants. ',
            options: { color: COL.textLight, bold: true },
          },
          { text: 'Your share depends on ', options: { color: COL.textMuted } },
          { text: 'engagement units', options: { color: COL.mint, bold: true } },
          {
            text: ': free downloads count 1\u00D7, paid-subscriber downloads count 5\u00D7, each \u20AC1 of fan-sub revenue counts 1\u00D7. ',
            options: { color: COL.textMuted },
          },
          {
            text: 'Passive listening doesn\u2019t count. ',
            options: { color: COL.textLight, bold: true },
          },
    ...
    s.addText(
      [
        {
          text: 'TOTAL OVER 3 YEARS:  ',
          options: { color: COL.amber, bold: true, charSpacing: 3 },
        },
        {
          text: '~\u20AC312,000 in artists\u2019 hands \u2014 ',
          options: { color: COL.textLight, bold: true },
        },
        {
          text: '46% more than a model where the org takes a cut. Yours because of how we routed the money, not because we generated more of it.',
          options: { color: COL.textMuted, italic: true },
        },
      ],
      {
  ...
  // ─── Slide: Tahti Radio + venue calendar (NEW v6) ─────────────
  {
    const s = pres.addSlide()
    s.background = { color: COL.bg }
    ...
    s.addText(
      'Tahti Radio amplifies your live broadcasts. Venue calendars surface where you\u2019re playing.',
      {
        x: 0.7,
        y: 1.15,
        w: 12,
    ...
    const features = [
      {
        ...
        icon: FaBroadcastTower,
        head: 'Tahti Radio',
        sub: 'The org-operated 24/7 meta-stream',
        body: [
          {
            text: 'When you go live, Tahti Radio may relay your broadcast \u2014 along with whichever other channels are currently on air. ',
            options: { color: COL.textLight, bold: true },
          },
          {
            text: 'Fair rotation, no curation, no editorial control. ',
            options: { color: COL.textMuted },
          },
          { text: 'Multistreamed to Mixcloud Live ', options: { color: COL.cyan, bold: true } },
          {
            text: 'so listeners can find you from outside the platform. Opt-out toggle in your settings if you prefer.',
            options: { color: COL.textMuted },
          },
        ],
        rows: [
      ...
      },
      {
        ...
        color: COL.amber,
        icon: FaBuilding,
        head: 'Venue calendars',
        sub: 'Where you\u2019re playing, when, for whom',
        body: [
          {
            text: 'Venues can register on  Tahti and publish calendars of broadcasts at their location. ',
            options: { color: COL.textLight, bold: true },
          },
          {
            text: 'Your future gigs at a venue appear on the venue\u2019s page; their iCalendar feed shows up in your fans\u2019 calendar apps. ',
            options: { color: COL.textMuted },
          },
          { text: 'Not a booking marketplace ', options: { color: COL.amber, bold: true } },
          {
            text: '\u2014 venues find artists, artists find venues, we just publish the schedule.',
            options: { color: COL.textMuted },
          },
        ],
        rows: [
  ...
  // ─── Slide 10: pricing ──────────────────────────────────────────
  {
    ...
    s.addText('One paid tier. Free users get a complete product \u2014 not a frustrating trial.', {
      x: 0.7,
      y: 1.15,
      w: 12,
      h: 0.5,
    ...
    const tiers = [
      {
        ...
        tag: 'FREE',
        price: '\u20AC0',
        sub: 'Start here. No upgrade pressure.',
        emphasize: false,
        features: [
          'Your channel + profile + releases',
          'MP3 192 kbps audio quality',
          '1 hour of live broadcasting per week',
          'Live chat with announcements',
          'Downloads enabled (free + paid)',
          'Fan-subscriptions enabled',
          'Smart links, embed widget',
          'Soft 500 MB storage \u2014 not enforced',
        ],
      },
      {
        icon: FaCrown,
        ...
        tag: 'TAHTI',
        price: '\u20AC40',
        sub: '/year \u2014 lossless, unlimited live',
        emphasize: true,
        features: [
          'Everything in Free, plus:',
          'Lossless FLAC streaming for all listeners',
          'FLAC downloads available',
          'Unlimited live broadcasting time',
          'Unlimited multistream destinations',
          'Mixcloud auto-upload + DSP distribution',
          'Newsletter, social auto-post',
          'Custom domain, API access',
          'Member of the association \u2014 eligible for grants',
        ],
      },
    ]
    ...
    for (let i = 0; i < tiers.length; i++) {
      ...
      const x = 2.45 + i * 4.3
      const cardW = 4.0
      const cardH = t.emphasize ? 5.4 : 5.2
      const cardY = t.emphasize ? 1.85 : 1.95
      ...
      s.addShape(pres.shapes.RECTANGLE, {
        ...
        h: cardH,
        fill: { color: COL.card },
        line: { color: t.emphasize ? t.color : COL.cardBorder, width: t.emphasize ? 2 : 1 },
      ...
      const ic = await iconPng(t.icon, t.color, 256)
      s.addImage({ data: ic, x: x + 0.3, y: cardY + 0.25, w: 0.6, h: 0.6 })
      s.addText(t.tag, {
        x: x + 1.0,
        y: cardY + 0.22,
        w: 2.4,
    ...
    // Friendly upgrade footer
    s.addShape(pres.shapes.RECTANGLE, {
      x: 0.7,
      y: 7.3,
      w: 11.9,
      h: 0.0, // hidden divider line, just for visual rhythm
      fill: { color: COL.bg },
      line: { color: COL.bg },
    })
  }
  ...
  await pres.writeFile({ fileName: '/home/claude/tahti-package-v7/slides/Tahti-Community.pptx' })
  console.log('OK')
}
