...
async function main() {
  ...
  // ─── Slide 2b: the structural problems with each ────────────────
  {
    ...
    s.addText("It\u2019s worse than just scattered.", {
      ...
      fontFace: F.header, fontSize: 36, bold: true, color: COL.textLight, margin: 0,
    });
    s.addText("Each platform has a structural problem you can\u2019t fix by trying harder.", {
      x: 0.7, y: 1.15, w: 12, h: 0.5,
      fontFace: F.body, fontSize: 14, italic: true, color: COL.textMuted, margin: 0,
    });

    const critiques = [
      {
        x: 0.7, color: COL.rose, head: "SoundCloud & Mixcloud",
      ...
      {
        x: 4.75, color: COL.amber, head: "Spotify",
      ...
      {
        x: 8.80, color: COL.cyan, head: "Bandcamp",
    ...
    for (const c of critiques) {
      s.addShape(pres.shapes.RECTANGLE, {
        x: c.x, y: 1.95, w: 3.85, h: 4.55,
        fill: { color: COL.card }, line: { color: COL.cardBorder, width: 1 }, shadow: sh(0.35),
      });
      s.addShape(pres.shapes.RECTANGLE, {
        x: c.x, y: 1.95, w: 3.85, h: 0.08, fill: { color: c.color }, line: { color: c.color },
      });
      s.addText(c.head, {
        x: c.x + 0.25, y: 2.2, w: 3.4, h: 0.5,
        fontFace: F.header, fontSize: 18, bold: true, color: c.color, margin: 0,
      });
      s.addText(c.sub, {
        x: c.x + 0.25, y: 2.7, w: 3.4, h: 0.8,
        fontFace: F.body, fontSize: 12, italic: true, color: COL.textLight, margin: 0, valign: "top",
      });
      s.addText(c.body, {
        x: c.x + 0.25, y: 3.55, w: 3.4, h: 2.85,
    ...
    s.addText([
      { text: "WHAT TAHTI DOES DIFFERENTLY:  ", options: { color: COL.mint, bold: true, charSpacing: 3 } },
      { text: "Lossless audio for paid \u2014 your listeners hear what you made. No algorithm, no AI feed. Broadcasting + chat + downloads, not just storage.", options: { color: COL.textMuted, italic: true } },
  ...
  // ─── Slide 8: how money reaches you (engagement units + fan-subs) ─
  {
    ...
    const flows = [
      ...
      },
      {
        ...
        fig: "\u20AC150k", figLabel: "to artists, 3yr total",
      },
    ];
    ...
    s.addText([
      ...
      { text: "~\u20AC312,000 in artists\u2019 hands \u2014 ", options: { color: COL.textLight, bold: true } },
      { text: "46% more than a model where the org takes a cut. Yours because of how we routed the money, not because we generated more of it.", options: { color: COL.textMuted, italic: true } },
  ...
  // ─── Slide 10: pricing ──────────────────────────────────────────
  {
    ...
    const tiers = [
      {
        ...
        sub: "Start here. No upgrade pressure.", emphasize: false,
        features: [
      ...
      {
        icon: FaCrown, color: COL.cyan, tag: "TAHTI", price: "\u20AC40",
        sub: "/year \u2014 lossless, unlimited live", emphasize: true,
        features: [
    ...
    for (let i = 0; i < tiers.length; i++) {
      ...
      if (t.emphasize) {
        s.addShape(pres.shapes.RECTANGLE, {
          x: x + cardW - 1.5, y: cardY + 0.15, w: 1.35, h: 0.4,
          fill: { color: t.color }, line: { color: t.color },
        });
        s.addText("RECOMMENDED", {
          x: x + cardW - 1.5, y: cardY + 0.15, w: 1.35, h: 0.4, align: "center",
          fontFace: F.header, fontSize: 10, bold: true, color: COL.bg, charSpacing: 2, valign: "middle", margin: 0,
        });
      }
      const ic = await iconPng(t.icon, t.color, 256);
      s.addImage({ data: ic, x: x + 0.3, y: cardY + 0.25, w: 0.6, h: 0.6 });
      s.addText(t.tag, {
        x: x + 1.0, y: cardY + 0.22, w: 2.4, h: 0.6,
        fontFace: F.header, fontSize: 16, bold: true, color: t.color, charSpacing: 4, margin: 0, valign: "middle",
      });
      s.addText(t.price, {
        x: x + 0.3, y: cardY + 0.95, w: 3.5, h: 0.95,
        fontFace: F.header, fontSize: 50, bold: true, color: COL.textLight, margin: 0,
      });
      s.addText(t.sub, {
        x: x + 0.3, y: cardY + 1.9, w: 3.5, h: 0.4,
        fontFace: F.body, fontSize: 11, italic: true, color: COL.textMuted, margin: 0,
      });
      const items = t.features.map((f, idx) => ({
        text: f,
        options: { bullet: { code: "25AA" }, color: COL.textMuted, fontSize: 12, breakLine: idx < t.features.length - 1 },
      }));
      s.addText(items, {
        x: x + 0.3, y: cardY + 2.4, w: 3.5, h: 2.85,
        fontFace: F.body, paraSpaceAfter: 4, margin: 0, valign: "top",
      });
    }

    // Friendly upgrade footer
    s.addShape(pres.shapes.RECTANGLE, {
      x: 0.7, y: 7.3, w: 11.9, h: 0.0,  // hidden divider line, just for visual rhythm
