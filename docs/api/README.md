...
| Collections and feeds         | `GET /api/v1/collections/:slug` (nulls `sound.audioUrl` + sets `gate` when the viewer is not entitled), RSS at `.../rss.xml` (omits `<enclosure>` for non-FREE sounds), `GET /api/v1/u/:username/rss.xml`                                                                                                                                                                                                                                                                   |
| Collection subscribe          | `POST /api/v1/collections/:slug/subscribe`                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Smart links                   | `GET /api/v1/r/:slug` (nulls `audioUrl` + `gate` when a track is linked to a gated Sound), click tracking via `POST /api/smartlink/click`                                                                                                                                                                                                                                                                                                                                   |
| Public releases/tracks        | `GET /api/v1/releases/:id`, `GET /api/tracks/:id` and `GET /api/channels/:slug/items` (null `audioUrl` + `gate` when the viewer is not entitled), discover latest-tracks / Selects gallery / new-to-you / feed (same gate), embed play `GET /api/v1/embed/col/:slug/tracks/:trackId/play` (403 when gated), downloads via `GET /api/v1/c/:slug/archive/:itemId/download` (same purchase/subscriber gate) and `GET /api/v1/releases/:smartLinkSlug/tracks/:trackId/download` |
