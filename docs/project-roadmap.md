...
| **M12** Profile + releases | 🟡 Partial | Release CRUD, smart links, DSP editor, **profile playback** (`archiveItemId` + `streamKey` presign); **cover art upload to MinIO** (`artworkKey` + presigned URLs); **JSON-LD + ISR 60s** on `/u/:username`; **sitemap** (`/api/sitemap/*.xml`, Next `sitemap.ts`); **press kit JSON** (`/api/v1/u/:username/press-kit.json`); **CSV bulk import** (`POST /api/me/releases/import`). Deferred: none |
