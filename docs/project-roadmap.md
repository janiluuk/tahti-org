...
| **M18** Downloads first-class | 🟡 Partial | Archive + release downloads, fraud-scan cron, Tor/datacenter CIDR; **daily Tor Redis sync** + **CI freshness check** on bundled `tor-exit-cidrs.txt` (`pnpm tor-exit:check`). Deferred: automated sync in deploy cron |
