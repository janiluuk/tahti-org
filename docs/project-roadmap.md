...
| **M10** Member governance | ✅ Done | `Motion`/`Vote` models, `requireMember`/`requireBoard` guards, advisory voting (Topic 11), members `/governance` portal, tally hidden until close, `MotionComment` discussion thread (2026-07-08 — the DRAFT-state "Discussion" label finally has a real thread behind it) |
...
| [x] | Platform-level content moderation / abuse-reporting queue (flag + review, tracks/releases/channels/collections) | Minimal MVP, 2026-07-07: `ContentReport` model (targetType/targetId, no FK — reports a channel, release, archive item, or collection by id/slug); anonymous public `POST /api/v1/reports` (no account required, matching the listener privacy model; IP-hashed with the same daily-rotating-salt pattern as `Download.byIpHash`; rate-limited 5/hour/IP); board-only `/admin/content-reports` queue with status filter (OPEN/REVIEWING/ACTIONED/DISMISSED) and inline resolve actions; a reusable `<ReportButton>` wired onto the channel and profile pages. Deliberately minimal — no automated takedown, resolution is a manual admin action using existing tools (artist deletion, admin suspend). `<ReportButton>` coverage completed 2026-07-22: release smart-link and public collection pages were already wired by then; added the one remaining gap, per-track `ARCHIVE_ITEM` reporting, to both the channel page's track rows and the public collection page's track rows. | M21 follow-up |
...
| Status | ID | Description | Size | Priority |
|:---:|:---|---|:---:|:---:|
| [x] | **PLAT-080** | **Google Drive import (phase 1)** — OAuth connect in dashboard upload flow (`_google-drive-connect.tsx`); Google Picker integration (`_google-drive-picker.tsx`); server-side fetch straight to MinIO via `apps/worker/src/jobs/cloud-import-google-drive.ts` (no browser download). Corrected 2026-07-07 gap-analysis audit — this was fully implemented, roadmap status was stale. | Large | P2 |
| [ ] | **PLAT-081** | **Cloud import abstraction** — Shared `CloudImportProvider` interface (`listFiles`, `getDownloadStream`, `revokeToken`) so Dropbox, OneDrive, or Box can plug in later without duplicating upload plumbing. Evaluate [Open Cloud Mesh](https://ocm.cern.ch/) / WebDAV as a fallback for self-hosted Nextcloud. Google Drive is still a bespoke integration, same pattern as Mixcloud/Bandcamp/SoundCloud — this abstraction genuinely doesn't exist yet. | Medium | P3 |
| [~] | **PLAT-082** | **Import job UX** — Status-pill progress tracking (QUEUED/DOWNLOADING/DONE/FAILED) exists in `_google-drive-picker.tsx`. Not verified: a dedicated audit-log entry per import (`source: google_drive`, file id, bytes) — no `auditLog`/`AuditLog` reference found in the worker job. | Medium | P2 |
| [x] | **PLAT-083** | **Security & privacy** — Refresh tokens stored encrypted via `encryptStreamKey` (same pattern as Mixcloud OAuth), `googleDriveAccessTokenEnc`/`googleDriveRefreshTokenEnc` on `User`; `DELETE` route exists to disconnect. Corrected 2026-07-07 gap-analysis audit — roadmap status was stale. | Small | P1 |
...
| [x] | **PERF-007** | **Visual preset picker** — static CSS thumbnails for unselected cards, `ChannelVisualizer` (live WebGL) now mounts only for the selected preset. Was rendering 4 simultaneous WebGL contexts + RAF loops per picker (channel/release/archive-item settings each show one). Fixed 2026-07-08. | P2 |
| [x] | **PERF-008** | **Paginate** releases, stash, newsletter drafts, programme list endpoints. Releases/stash/newsletter-drafts got real `{page,limit,total,<items>}` pagination matching the existing admin-list convention (default `limit=100`, preserving today's "show everything" UX for realistic per-artist volumes while bounding worst-case query cost). Programme got a `take: 500` safety cap instead — its rotation editor filters the full list client-side to derive in-rotation/available, so real pagination would break that UX. Fixed 2026-07-08. | P2 |
