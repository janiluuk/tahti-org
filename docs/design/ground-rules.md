...
| `/admin/governance/audit` | Same pattern as `/admin/grants`: 50 rows/page with Previous/Next pagination, not an unbounded list. Topic chips + action filter sit in the header; the shared `LogViewer` is the part that's tall, by design. |
