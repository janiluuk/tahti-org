# Replay ry — governance and legal framework

## Legal form

**Replay ry** — registered nonprofit association (*rekisteröity yhdistys*) under
...
The bylaws (*säännöt*) are the governing document. Key clauses for Replay ry:
...
- Replay ry takes **0% of fan-to-artist subscription revenue**
...
1. **Anyone can use, modify, distribute the code.** Anyone can run their own
   Replay clone. Encourage this — it's the point.
...
4. **Forks are fine, friendly even.** If someone wants to run Replay-flavored for
...
## Funding sources Replay ry is eligible for
