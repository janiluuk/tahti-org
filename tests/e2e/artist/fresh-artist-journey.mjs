 ...
 *   SCREENSHOT_THEME=light node tests/e2e/artist/fresh-artist-journey.mjs   # or 'dark' (default)
 *
 * Requires Docker stack + seed (./scripts/e2e-screenshots.sh --capture after seed).
 * Screenshots: docs/e2e-screenshots/artist/journey/<light|dark>/
 */
...
// Browser prefers-color-scheme emulation, NOT the product's per-artist channel
// theme picker — see ../lib/journey-capture.mjs for why the two are distinct.
const THEME = process.env.SCREENSHOT_THEME ?? 'dark'
const OUT = join(__dirname, '../../../docs/e2e-screenshots/artist/journey', THEME)
...
async function main() {
  ...
  try {
    ...
    if (!hasChannel) {
      // Channel setup wizard — 5 steps (Identity, Genres & listing, Look,
      // Rotation, Go live; see apps/web/src/app/dashboard/setup-channel/).
      // Only step 1 creates the channel; steps 3-4 are skippable via their
      // own "Skip for now" links when there's nothing to configure yet.
      ...
      await shot(page, '02-setup-channel.png', 'setup channel wizard step 1 (identity)')

      await page.locator('#setup-description').fill('Fresh journey artist — e2e demo station.')
      await page.getByRole('button', { name: 'Continue' }).click()
      await page.waitForURL((url) => url.searchParams.get('step') === '2', { timeout: 45_000 })
      ok('channel provisioned via UI (step 1 complete)')
      await page.waitForTimeout(500)
      await shot(page, '02b-setup-channel-genres.png', 'setup channel wizard step 2 (genres)')

      await page.getByRole('button', { name: 'Continue' }).click()
      await page.waitForURL((url) => url.searchParams.get('step') === '3', { timeout: 45_000 })

      await page.getByRole('link', { name: 'Skip for now →' }).click()
      await page.waitForURL((url) => url.searchParams.get('step') === '4', { timeout: 45_000 })

      const skipRotation = page.getByRole('link', { name: 'Skip for now →' })
      if (await skipRotation.count()) {
        await skipRotation.click()
        await page.waitForURL((url) => url.searchParams.get('step') === '5', { timeout: 45_000 })
      }
      ok('setup wizard walked through to step 5 (go live)')
    } else {
      ok('channel already exists — skipping provision UI')
      // setup-channel server-redirects to /dashboard/channel/edit when a
      // channel already exists and no ?step= is present — wait for that
      // navigation to fully settle before moving on.
      await page.goto(`${APP}/dashboard/setup-channel`, { waitUntil: 'load', timeout: 45_000 })
    ...
    // Sell side of paid content: fan-tier / paid-content settings, where the
    // artist configures what listeners get by subscribing (see
    // listener-journey.mjs for the matching buy side).
    await page.goto(`${APP}/dashboard/settings/fan-subs`, { waitUntil: 'load', timeout: 45_000 })
    await assertAuthenticated(page, 'fan-subs settings')
    await page.waitForTimeout(600)
    await shot(page, '04b-fan-subs-settings.png', 'fan-subs settings (sell side)')
    ...
    const manifest = [
      ...
      { file: '04b-fan-subs-settings.png', label: 'Fan-subs settings (sell side of paid content)' },
