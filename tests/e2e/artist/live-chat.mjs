...
async function main() {
  ...
  try {
    ...
    if (connectRes.ok) {
      ...
      console.log(
        `⚠ Icecast on_connect returned ${connectRes.status} — live simulation skipped (cap or fixture)`,
      )
    }
  } catch (e) {
    ...
    if (wentLive) {
      for (const [page, label] of [
        ...
        if ((await player.count()) > 0) ok(`${label} sees the live stream player`)
          ...
          console.log(
            `⚠ ${label} — live player element not present (HLS pipeline may be unavailable)`,
          )
      }
    } else {
