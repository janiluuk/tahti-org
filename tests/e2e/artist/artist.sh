...
# Merged from the former journeys/dashboard-player.sh (redundant release/archive/
# dashboard checks folded into run_artist_journey above) — kept the checks unique
# to the archive/player surface: download-gate stats, public channel items +
# audioUrl, and embed metadata.
run_artist_player_journey() {
  echo ""
  echo "── Artist dashboard & player journey (API) ────────────────"
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  ...
  source "$JOURNEYS_DIR/../journeys-fixtures.sh"
