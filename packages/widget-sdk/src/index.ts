...
// The only thing a Disco-widget bundle imports. Bundle it in (the sandbox
...
import type { DiscoWidgetHostApi, DiscoWidgetModule } from './types.js'

export type { DiscoWidgetHostApi, DiscoWidgetModule } from './types.js'

/** Sets up the postMessage bridge to the host page. Call once, at module load. */
export function createHostApi(): DiscoWidgetHostApi {
  ...
  post({ source: DISCO_WIDGET_MESSAGE_SOURCE, type: 'ready' })
  ...
  return {
    ...
    resize(heightPx: number) {
      post({ source: DISCO_WIDGET_MESSAGE_SOURCE, type: 'resize', height: heightPx })
    },
    openLink(url: string) {
      post({ source: DISCO_WIDGET_MESSAGE_SOURCE, type: 'open-link', url })
    },
...
export function mountDiscoWidget(module: DiscoWidgetModule): void {
  const host = createHostApi()
  const container = document.getElementById(DISCO_WIDGET_ROOT_ELEMENT_ID) ?? document.body
