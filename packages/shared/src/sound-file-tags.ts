...
import { SOUND_GENRES, SOUND_METADATA_DEFAULTS } from './dto/sound-metadata.js'

export type ParsedSoundFileTags = {
  bpm: number | null
  key: string | null
  genre: string | null
...
/** Normalize ffprobe ID3/Vorbis tag map into sound metadata fields. */
export function parseSoundFileTags(tags: Record<string, unknown>): ParsedSoundFileTags {
  return {
 ...
 * SOUND_GENRES — same normalization either way, so an import from Spotify/
...
export function mapGenre(raw: string): { genre?: string; genreCustom?: string } {
  ...
  if (!trimmed) return {}
  const preset = SOUND_GENRES.find((g) => g.toLowerCase() === trimmed.toLowerCase())
  if (preset) return { genre: preset }
  return { genreCustom: trimmed }
}

export type SoundForTagMerge = {
  description: string | null
  genre: string | null
...
export function mergeParsedSoundTags(
  embedded: ParsedSoundFileTags,
  acoustic: Pick<ParsedSoundFileTags, 'bpm' | 'key'>,
): ParsedSoundFileTags {
  return {
    ...embedded,
 ...
 */
export function mergeDetectedSoundMetadata(
  item: SoundForTagMerge,
  detected: ParsedSoundFileTags,
  ...
  const genreUnset =
    ...
    (!item.genre?.trim() || item.genre === SOUND_METADATA_DEFAULTS.genre)
