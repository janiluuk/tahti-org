...
describe('soundPlaybackKey', () => {
  it('prefers flac when both exist (STREAM-011: lossless by default)', () => {
    expect(soundPlaybackKey({ mp3Key: 'a.mp3', flacKey: 'a.flac' })).toBe('a.flac')
  })

  it('uses mp3 when flac is absent', () => {
    expect(soundPlaybackKey({ mp3Key: 'a.mp3', flacKey: null })).toBe('a.mp3')
  })

  it('returns null when neither exists', () => {
    expect(soundPlaybackKey({ mp3Key: null, flacKey: null })).toBe(null)
  })
})
