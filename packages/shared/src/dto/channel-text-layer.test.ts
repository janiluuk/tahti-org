...
describe('channel text layer DTO', () => {
  ...
  it('rejects empty text when provided', () => {
    expect(() =>
      ChannelTextLayerPatchSchema.parse({ textLayerText: 'a'.repeat(121) }),
    ).toThrow()
  })

  it('detects active text layer', () => {
    expect(
      isActiveTextLayer({ textLayerMode: 'COSMIC_NEON', textLayerText: 'Live now' }),
    ).toBe(true)
