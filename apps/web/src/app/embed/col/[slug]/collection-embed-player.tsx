...
export default function CollectionEmbedPlayer({ collection }: { collection: CollectionEmbed }) {
  ...
  async function playTrack(track: Track) {
    ...
    try {
      const res = await fetch(
        `${apiUrl}/api/v1/embed/col/${collection.slug}/tracks/${track.id}/play`,
        { cache: 'no-store' },
      )
    ...
    <div>
      <div className="embed-header">
        {collection.coverUrl && (
          ...
          <img src={collection.coverUrl} alt="" className="embed-artwork" width={56} height={56} />
        )}
        <div className="embed-header__meta">
          <p className="embed-header__title">{collection.name}</p>
          <Link
          ...
          >
            {collection.artist.displayName} on Tahti
          </Link>
        </div>
      </div>
