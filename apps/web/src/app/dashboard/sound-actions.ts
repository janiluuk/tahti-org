...
export async function prepareSoundBannerUpload(
  itemId: string,
  body: { filename: string; contentType: string },
): Promise<{ uploadKey?: string; uploadUrl?: string; error: string | null }> {
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/banner/prepare`, {
    method: 'POST',
...
export async function completeSoundBannerUpload(
  itemId: string,
  uploadKey: string,
): Promise<{ url?: string | null; error: string | null }> {
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/banner/complete`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Cookie: sessionHeader() },
...
export async function fetchSoundBannerFromUrl(
  itemId: string,
  sourceUrl: string,
): Promise<{ url?: string | null; error: string | null }> {
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/banner/from-url`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Cookie: sessionHeader() },
...
export async function deleteSoundItem(id: string): Promise<{ error: string | null }> {
  const res = await fetch(`${apiUrl}/api/me/sound/${id}`, {
    method: 'DELETE',
...
export async function updateSoundMetadata(
  id: string,
...
export async function fetchSoundVersions(
  itemId: string,
): Promise<{ versions?: import('@tahti/shared').SoundVersionRow[]; error: string | null }> {
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/versions`, {
    headers: { Cookie: sessionHeader() },
    cache: 'no-store',
  })
  if (!res.ok) {
...
export async function prepareSoundVersionUpload(
  itemId: string,
  body: { filename: string; contentType: string },
): Promise<{ uploadId?: string; uploadUrl?: string; error: string | null }> {
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/versions/prepare`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Cookie: sessionHeader() },
    body: JSON.stringify(body),
    cache: 'no-store',
...
export async function completeSoundVersionUpload(
  itemId: string,
  ...
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/versions/complete`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Cookie: sessionHeader() },
    body: JSON.stringify(body),
    cache: 'no-store',
...
export async function activateSoundVersion(
  ...
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/versions/${versionId}/activate`, {
    method: 'POST',
    headers: { Cookie: sessionHeader() },
    cache: 'no-store',
  })
...
export async function fetchSoundEditListDraft(itemId: string): Promise<{
  ...
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/editor/draft`, {
    headers: { Cookie: sessionHeader() },
    cache: 'no-store',
  })
  if (!res.ok) {
...
export async function saveSoundEditListDraft(
  itemId: string,
  editList: import('@tahti/audio-edit').EditList,
...
export async function renderSoundEditList(
  itemId: string,
  body: {
    editList: import('@tahti/audio-edit').EditList
    versionLabel: string
  ...
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/editor/render`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Cookie: sessionHeader() },
    body: JSON.stringify(body),
    cache: 'no-store',
...
export async function createSoundClip(
  itemId: string,
  body: {
    startSec: number
    endSec: number
  ...
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/editor/create-clip`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Cookie: sessionHeader() },
    body: JSON.stringify(body),
    cache: 'no-store',
...
export async function fetchSoundVersionDownloadUrl(
  itemId: string,
  versionId: string,
): Promise<{ url?: string; contentType?: string; error: string | null }> {
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/versions/${versionId}/download`, {
    headers: { Cookie: sessionHeader() },
    cache: 'no-store',
  })
  if (!res.ok) {
...
export async function fetchSoundVersion(
  itemId: string,
  versionId: string,
): Promise<{ version?: import('@tahti/shared').SoundVersionRow; error: string | null }> {
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/versions/${versionId}`, {
    headers: { Cookie: sessionHeader() },
    cache: 'no-store',
  })
  if (!res.ok) {
...
export async function waitForSoundVersionReady(
  itemId: string,
  versionId: string,
  maxAttempts = 90,
): Promise<{
  ready: boolean
  version?: import('@tahti/shared').SoundVersionRow
  error: string | null
}> {
  for (let i = 0; i < maxAttempts; i++) {
    const res = await fetchSoundVersion(itemId, versionId)
...
export async function fetchSoundEditorSource(itemId: string): Promise<{
  url?: string
  ...
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/editor/source`, {
    headers: { Cookie: sessionHeader() },
    cache: 'no-store',
  })
  if (!res.ok) {
...
export async function publishSoundToRelease(
  ...
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/editor/publish-to-release`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Cookie: sessionHeader() },
    body: JSON.stringify(body),
    cache: 'no-store',
...
export async function fetchDownloadGateStats(itemId: string): Promise<{
  ...
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/download-gate-stats`, {
    headers: { Cookie: sessionHeader() },
    cache: 'no-store',
  })
  if (!res.ok) {
...
export async function fetchSoundDownloadUrl(
  itemId: string,
): Promise<{ url: string; filename: string } | { error: string }> {
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/download`, {
    headers: { Cookie: sessionHeader() },
    cache: 'no-store',
  })
  if (!res.ok) {
...
export async function fetchSoundStems(
  itemId: string,
): Promise<{ jobs: StemJobRow[] } | { error: string }> {
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/stems`, {
    headers: { Cookie: sessionHeader() },
    cache: 'no-store',
  })
...
export async function requestSoundStems(
  ...
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/stems/render`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Cookie: sessionHeader() },
...
export async function exportSoundToHearthis(
  ...
  const res = await fetch(`${apiUrl}/api/me/sound/${itemId}/export/hearthis`, {
    method: 'POST',
    headers: { Cookie: sessionHeader() },
    cache: 'no-store',
  })
