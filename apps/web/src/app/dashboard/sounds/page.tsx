...
export default async function SoundPage() {
  ...
  const [soundsForEdit, sounds, releasesRes, collectionsRes, mixcloudStatus] = await Promise.all([
    slug
      ...
      : Promise.resolve([]),
    slug
      ? apiFetch<SoundItem[]>(apiUrl, cookie, `/api/channels/${slug}/items`)
      : Promise.resolve([]),
  ...
  const items = soundsForEdit ?? []
  const playable = sounds ?? []
