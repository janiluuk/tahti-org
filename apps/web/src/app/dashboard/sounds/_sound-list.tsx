...
const SoundEditor = dynamic(() => import('../sound-editor'))

type SoundListItem = Record<string, unknown> & {
  id: string
  title: string
  status: string
  isPublic?: boolean
...
function itemFilter(item: SoundListItem): 'unpublished' | 'drafts' | 'published' {
...
export function SoundList({
  ...
  return (
    <LibraryBrowser
      ...
      getDurationSec={(item) => item.durationSec}
      getBpm={(item) => item.bpm}
      getGenre={(item) => item.genre}
      getStatus={itemFilter}
      searchPlaceholder="Search archive…"
      noMatchMessage="No recordings match."
      toolbarExtra={
        showEmbedFilter && embedCount > 0 ? (
          <label className="sound-list__embed-filter">
                  ...
                  >
                    <div className="sound-list__cover">
                      {cover ? (
                        ...
                        <div
                          className={`sound-list__cover-ph cover-gradient--${placeholderGradients.get(item.id) ?? 'aurora'}`}
                          aria-hidden
                        />
                      )}
                      {providerLabel && (
                    ...
                    <div className="sound-list__row-body">
                      <SoundEditor
                        item={item}
