...
export function SoundDownloadPanel({ itemId }: { itemId: string }) {
  ...
  const refresh = useCallback(async () => {
    const result = await fetchSoundStems(itemId)
    if ('error' in result) {
  ...
  // comes from the server (SoundItemStemJob), not local component state.
  useEffect(() => {
  ...
  async function onDownloadOriginal() {
    ...
    setError(null)
    const result = await fetchSoundDownloadUrl(itemId)
  ...
  async function onRequestStems(stemSet: StemJobRow['stemSet']) {
    ...
    const result = await requestSoundStems(itemId, stemSet)
    if (result.error) {
      setError(result.error)
      await refresh()
    }
