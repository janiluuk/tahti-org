...
import { createInternetRadioPreset, deleteInternetRadioPreset, updateInternetRadioPreset } from './actions'
  ...
  return (
    <div className="ui-panel studio-mt-sm">
      <div className="admin-row" style={{ justifyContent: 'space-between' }}>
        <div>
          <strong>{preset.name}</strong> {preset.genre && <Badge variant="neutral">{preset.genre}</Badge>}
          {preset.description && <p className="admin-text-muted">{preset.description}</p>}
        </div>
        <Button type="button" variant="danger" size="sm" disabled={pending} onClick={() => onRemove(preset.id)}>
          Delete
        </Button>
      </div>
      <div className="admin-row studio-mt-sm" style={{ gap: '0.5rem' }}>
        ...
        <Input
          ...
          disabled={pending}
        />
        <Button type="button" variant="secondary" size="sm" disabled={pending} onClick={() => void handleSave()}>
          Save
        </Button>
      </div>
...
export function AdminInternetRadioPanel({ initialPresets }: { initialPresets: InternetRadioPreset[] }) {
  ...
  return (
    <div>
      <Panel title="Add a preset station">
        ...
        <Field label="Genre">
          <Input value={genre} onChange={(e) => setGenre(e.target.value)} placeholder="Pop / Hits" />
        </Field>
      ...
      {presets.length === 0 ? (
        ...
        presets.map((p) => (
          <PresetRow
            key={p.id}
            preset={p}
            onChange={(updated) => setPresets((prev) => prev.map((pr) => (pr.id === updated.id ? updated : pr)))}
