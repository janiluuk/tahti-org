...
export default async function StatusPage() {
  ...
  return (
    <div className="status-page">
      <PublicPageHeader title="Tahti platform status" back={{ href: '/', label: '← Home' }}>
        Live health checks for the Tahti API and core services.
      </PublicPageHeader>
            ...
            <table className="status-page__table">
              ...
              <tbody>
                {Object.entries(data.checks).map(([id, check]) => (
                  <tr key={id}>
                    <td>{id}</td>
                    <td className={check.state === 'up' ? 'status-page__up' : 'status-page__down'}>
                      <span
