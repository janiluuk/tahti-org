...
export async function buyPurchaseTier(
  username: string,
  tierId: string,
  amountCents?: number,
  ...
  if (!sessionCookie) {
    return { error: 'Please log in to buy this.' }
  }

  const res = await fetch(
    `${apiUrl}/api/v1/u/${encodeURIComponent(username)}/purchase-tiers/${tierId}/checkout`,
    {
      method: 'POST',
      headers: {
  ...
  if (!res.ok) {
    return { error: (data as { error?: string }).error ?? 'Could not start checkout' }
  }
  return {
    error: null,
