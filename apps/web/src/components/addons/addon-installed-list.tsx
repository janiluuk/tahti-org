...
import type { AddonInstallView } from '@tahti/shared'
...
export interface AddonInstalledListProps {
  installs: AddonInstallView[]
  pendingId: string | null
  onToggle: (id: string, enabled: boolean) => void
  onReorder: (next: AddonInstallView[]) => void
...
export function AddonInstalledList({
  installs,
  pendingId,
  onToggle,
  onReorder,
  ...
  function openConfigure(install: AddonInstallView) {
  ...
  return (
    <SortableList
      as="div"
      className="addon-installed-list"
      items={installs}
      ...
      renderItem={(install, _index, sortable) => {
        ...
        return (
          <div
            ref={sortable.ref}
            className={`ui-panel addon-installed-row${sortable.isDragging ? ' is-dragging' : ''}`}
          >
            <div
            ...
            >
              <button
                ...
                type="button"
                className="addon-installed-row__handle"
            ...
            {isConfiguring && (
              <div className="addon-configure studio-mt-sm">
                <label className="studio-label" htmlFor={`widget-config-${install.id}`}>
                  Widget config (JSON)
                </label>
                <textarea
                  id={`widget-config-${install.id}`}
                  className="studio-input studio-textarea addon-configure__textarea"
                  rows={8}
                  value={draft}
                  onChange={(e) => {
