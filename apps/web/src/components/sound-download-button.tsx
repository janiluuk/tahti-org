...
export function SoundDownloadButton({
  ...
  const refreshGates = useCallback(async () => {
    ...
    const res = await fetch(
      `${API_URL}/api/v1/c/${encodeURIComponent(channelSlug)}/sounds/${itemId}/download-gates?fp=${encodeURIComponent(fp)}`,
      { credentials: 'include' },
    )
  ...
  async function acknowledgeRepost() {
    ...
    const fp = listenerFingerprint()
    const res = await fetch(
      `${API_URL}/api/v1/c/${encodeURIComponent(channelSlug)}/sounds/${itemId}/repost-ack`,
      {
        method: 'POST',
        credentials: 'include',
  ...
  async function download() {
    ...
    try {
      const fp = listenerFingerprint()
      const res = await fetch(
        `${API_URL}/api/v1/c/${encodeURIComponent(channelSlug)}/sounds/${itemId}/download?fp=${encodeURIComponent(fp)}`,
        { credentials: 'include' },
      )
      if (res.status === 401) {
        setShowLogin(true)
