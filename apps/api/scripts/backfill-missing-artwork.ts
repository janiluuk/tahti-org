...
/** Sound has no paletteJson extraction pipeline yet (by design — see
 * the field comment in schema.prisma) — only the missing-cover case applies. */
async function backfillSounds() {
  ...
  for (const item of items) {
    ...
    const key = `sound/${item.channel.user.username}/${item.id}/banner-${nanoid(8)}-generated.svg`
    if (!DRY_RUN) {
      ...
      await prisma.sound.update({ where: { id: item.id }, data: { bannerUrl } })
    }
    generated++
  }
...
async function main() {
  ...
  const sounds = await backfillSounds()

  console.log(JSON.stringify({ ok: true, dryRun: DRY_RUN, releases, collections, sounds }, null, 2))
}
