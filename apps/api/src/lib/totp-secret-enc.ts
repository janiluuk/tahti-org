...
// Separate key from stream-key-enc.ts by design — different secret domain.
...
const DEV_KEY_HEX = '11'.repeat(32)

function getKey(): Buffer {
  const hex = process.env.TOTP_SECRET_ENC_KEY ?? DEV_KEY_HEX
  const buf = Buffer.from(hex.slice(0, 64), 'hex')
  if (buf.length !== 32) throw new Error('TOTP_SECRET_ENC_KEY must be 32 bytes (64 hex chars)')
  return buf
}
