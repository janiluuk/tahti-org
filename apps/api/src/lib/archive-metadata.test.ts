...
import {
  effectiveBpm,
  effectiveKey,
  metadataForNewUpload,
} from './archive-metadata.js'

describe('archive-metadata', () => {
  it('prefers detected BPM/key when useDetectedBpmKey is true', () => {
    expect(
      effectiveBpm({ bpm: 120, bpmDetected: 128, useDetectedBpmKey: true }),
    ).toBe(128)
    expect(
      effectiveKey({ musicalKey: 'C', keyDetected: 'Am', useDetectedBpmKey: true }),
    ).toBe('Am')
  })

  it('uses manual BPM/key when useDetectedBpmKey is false', () => {
    expect(
      effectiveBpm({ bpm: 120, bpmDetected: 128, useDetectedBpmKey: false }),
    ).toBe(120)
    expect(
      effectiveKey({ musicalKey: 'C', keyDetected: 'Am', useDetectedBpmKey: false }),
    ).toBe('C')
  })
