...
const PREFIX = 'sound-like-test-'

describe('sound item like routes', () => {
  ...
  it('POST likes the track, bumps the count, and writes one SOUND_ITEM_LIKE audit row', async () => {
    const res = await app.inject({
      method: 'POST',
      url: `/api/v1/c/${channelSlug}/sounds/${itemId}/like`,
    ...
    const rows = await prisma.auditLog.findMany({
      where: { action: 'SOUND_ITEM_LIKE', actorId: likerId, targetId: itemId },
    })
  ...
  it('404s for an unknown sound item', async () => {
    const res = await app.inject({
      method: 'POST',
      url: `/api/v1/c/${channelSlug}/sounds/not-a-real-id/like`,
      headers: { cookie: likerCookie },
    })
