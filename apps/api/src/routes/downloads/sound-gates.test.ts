...
describe('M22 — repost/follow download gates', () => {
  ...
  beforeAll(async () => {
    ...
    const open = await prisma.sound.create({
      data: {
        channelId,
        title: 'Open mix',
        rawKey: 'raw/open.wav',
    ...
    const gated = await prisma.sound.create({
      data: {
        channelId,
        title: 'Gated mix',
  ...
  afterAll(async () => {
    ...
    await prisma.soundRepostAck.deleteMany({
      where: { soundId: { in: [itemId, gatedItemId] } },
    })
  ...
  it('blocks download until repost and follow requirements are met', async () => {
    const blocked = await app.inject({
      method: 'GET',
      url: `/api/v1/c/${SLUG}/sounds/${gatedItemId}/download?fp=gate-fp-1`,
      headers: dlHeaders,
    })
    expect(blocked.statusCode).toBe(403)

    const gateLog = await prisma.download.count({
      where: { soundId: gatedItemId, reason: { in: ['gate_repost', 'gate_follow'] } },
    })
    expect(gateLog).toBeGreaterThanOrEqual(1)

    await app.inject({
      method: 'POST',
      url: `/api/v1/c/${SLUG}/sounds/${gatedItemId}/repost-ack`,
