...
const downloadRoutes: FastifyPluginAsync = async (fastify) => {
  for (const url of DOWNLOAD_PATHS) {
    fastify.get(
      url,
      {
        schema: {
      ...
      async (request, reply) => {
        ...
        const parsedQuery = SoundDownloadQuerySchema.safeParse(request.query)
        ...
        const objectKey = wantSource
          ? (item.rawKey ?? item.flacKey ?? soundPlaybackKey(item))
          : wantFlac
            ? item.flacKey!
            : (soundPlaybackKey(item) ?? item.rawKey)
        if (!objectKey) {
        ...
        const weight =
          byUserId && (await isActiveFanSubscriber(fastify.prisma, channel.userId, byUserId))
            ? 5
            : 1
        ...
        if (dedupHit) {
          ...
          const countedForTrack = await fastify.prisma.download.count({
            where: { soundId: item.id, byFingerprint, countedAt: { not: null } },
          })
