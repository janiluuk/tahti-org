...
describe('M22 — repost/follow download gates', () => {
  ...
  it('blocks download until repost and follow requirements are met', async () => {
    ...
    expect(blocked.json().gates).toContain('repost')
    ...
    expect(stillBlocked.json().gates).toContain('follow')
  ...
  it('allows download without gates on open items', async () => {
    const res = await app.inject({
      method: 'GET',
      url: `/api/v1/c/${SLUG}/archive/${itemId}/download?fp=gate-fp-open`,
      headers: dlHeaders,
    })
