...
describe('M18 — sound downloads + engagement units', () => {
  ...
  it('returns 404 for an unknown channel', async () => {
    const res = await app.inject({
      method: 'GET',
      url: `/api/v1/c/no-such-channel/sounds/${itemId}/download?fp=abc`,
    })
  ...
  it('returns 400 for invalid download format query', async () => {
    const res = await app.inject({
      method: 'GET',
      url: `/api/v1/c/${SLUG}/sounds/${itemId}/download?format=wav`,
      headers: dlHeaders,
    })
  ...
  it('does not count the first download from a brand-new IP (24h threshold)', async () => {
    const res = await app.inject({
      method: 'GET',
      url: `/api/v1/c/${SLUG}/sounds/${itemId}/download?fp=fingerprint-new-ip`,
      headers: dlHeaders,
    })
  ...
  it('counts download when IP was first seen 24h+ ago', async () => {
    ...
    const res = await app.inject({
      method: 'GET',
      url: `/api/v1/c/${SLUG}/sounds/${itemId}/download?fp=fingerprint-A`,
      headers: dlHeaders,
    })
    expect(res.statusCode).toBe(200)
  ...
  it('does not count downloads with bot user agents', async () => {
    ...
    const res = await app.inject({
      method: 'GET',
      url: `/api/v1/c/${SLUG}/sounds/${itemId}/download?fp=bot-fp`,
  ...
  it('dedups a repeat download from the same fingerprint within 30 days', async () => {
    ...
    const first = await app.inject({
      method: 'GET',
      url: `/api/v1/c/${SLUG}/sounds/${itemId}/download?fp=fingerprint-B`,
      headers: dlHeaders,
    })
  ...
  it('rate-limits after 5 downloads in an hour (429)', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      const res = await app.inject({
        method: 'GET',
        url: `/api/v1/c/${SLUG}/sounds/${itemId}/download?fp=rate-${i}`,
        headers: dlHeaders,
      })
      expect(res.statusCode).toBe(200)
    }
    const blocked = await app.inject({
      method: 'GET',
      url: `/api/v1/c/${SLUG}/sounds/${itemId}/download?fp=rate-final`,
      headers: dlHeaders,
    })
