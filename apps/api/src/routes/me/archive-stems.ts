...
import {
  ArchiveDownloadResponseSchema,
  ArchiveStemsResponseSchema,
  IdParamSchema,
  RequestStemsSchema,
  openApiResponse,
  parseRouteParams,
...
const meArchiveStemsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  // Original file download — doesn't exist elsewhere in the archive routes.
  ...
  fastify.get(
    '/api/me/archive/:id/download',
    {
      ...
      schema: {
        ...
        response: openApiResponse(ArchiveDownloadResponseSchema, 'ArchiveDownload'),
      },
    },
    async (request, reply) => {
  ...
  fastify.get(
    '/api/me/archive/:id/stems',
    {
      ...
      schema: {
        ...
        response: openApiResponse(ArchiveStemsResponseSchema, 'ArchiveStems'),
      },
    },
    async (request, reply) => {
      const routeParams = parseRouteParams(IdParamSchema, request.params)
      ...
      const rows = await fastify.prisma.archiveItemStemJob.findMany({
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const existing = await fastify.prisma.archiveItemStemJob.findUnique({
        where: {
          archiveItemId_stemSet: { archiveItemId: routeParams.id, stemSet: parsed.data.stemSet },
        },
      })
      ...
      const stemJob = await fastify.prisma.archiveItemStemJob.upsert({
        where: {
          archiveItemId_stemSet: { archiveItemId: routeParams.id, stemSet: parsed.data.stemSet },
        },
        create: {
...
export default meArchiveStemsRoutes
