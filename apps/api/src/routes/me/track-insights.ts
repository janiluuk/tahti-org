...
import { buildSoundInsights, buildReleaseTrackInsights } from '../../lib/track-insights.js'

const meTrackInsightsRoutes: FastifyPluginAsync = async (fastify) => {
  // GET /api/me/sound/:id/insights — per-track stats for an Sound
  fastify.get(
    ...
    async (request, reply) => {
      ...
      const insights = await buildSoundInsights(fastify.prisma, user.id, routeParams.id, period)
