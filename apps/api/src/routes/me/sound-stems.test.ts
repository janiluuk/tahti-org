...
describe('sound stems + download', () => {
  ...
  beforeAll(async () => {
    ...
    const item = await createReadySound(prisma, owner.channel!.id, 'Stems Test Track')
    itemId = item.id
  })
  ...
  describe('GET /api/me/sound/:id/download', () => {
    it('requires auth', async () => {
      const res = await app.inject({ method: 'GET', url: `/api/me/sound/${itemId}/download` })
    ...
    it('returns a presigned URL for the best available source', async () => {
      const res = await app.inject({
        method: 'GET',
        url: `/api/me/sound/${itemId}/download`,
        headers: { cookie },
      })
  ...
  describe('POST /api/me/sound/:id/stems/render + GET /api/me/sound/:id/stems', () => {
    it('requires auth to request stems', async () => {
      const res = await app.inject({
        method: 'POST',
        url: `/api/me/sound/${itemId}/stems/render`,
    ...
    it('enqueues a stem job and reports PENDING status', async () => {
      ...
      expect(enqueueSeparateStems).toHaveBeenCalledWith(
        expect.objectContaining({ soundId: itemId, stemSet: 'TWO_STEM' }),
      )

      const statusRes = await app.inject({
        method: 'GET',
        url: `/api/me/sound/${itemId}/stems`,
        headers: { cookie },
      })
    ...
    it('lists presigned URLs for a READY job', async () => {
      await prisma.soundStemJob.updateMany({
        where: { soundId: itemId, stemSet: 'TWO_STEM' },
        data: {
          status: 'READY',
