...
const downloadGateStatsRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.get('/api/me/download-gate-stats', { preHandler: requireAuth }, async (request, reply) => {
    const user = request.sessionUser!
    const channel = await fastify.prisma.channel.findUnique({
      where: { userId: user.id },
      select: { id: true },
    })
    if (!channel) return reply.send({ items: [], totals: { repostAcks: 0, blockedAttempts: 0 } })
