...
export interface RenderSoundEditPayload {
  versionId: string
  soundId: string
  channelSlug: string
  sourceKey: string
...
export async function processRenderSoundEditJob(job: Job): Promise<void> {
  const data = job.data as RenderSoundEditPayload
  const {
    versionId,
    soundId,
    channelSlug,
  ...
  const version = await prisma.soundVersion.findUnique({ where: { id: versionId } })
  ...
  try {
    ...
    if (activate) {
      await prisma.$transaction([
        prisma.soundVersion.updateMany({
          where: { soundId },
          data: { isActive: false },
        }),
        prisma.soundVersion.update({
          where: { id: versionId },
          data: { isActive: true },
        }),
      ])
    ...
    await pruneSoundRevisions(prisma, soundId)
