...
export type SoundFallbackCacheSummary = {
  downloaded: number
  skipped: number
  pruned: number
}
...
export async function syncChannelSoundFallbackCache(
  prisma: PrismaClient,
  channelId: string,
  cacheRoot: string,
  maxItems = parseInt(process.env.SOUND_CACHE_MAX_ITEMS ?? '24', 10),
): Promise<SoundFallbackCacheSummary> {
  const summary: SoundFallbackCacheSummary = { downloaded: 0, skipped: 0, pruned: 0 }
  ...
  if (channel.fallbackEnabled || isTahtiRadio) {
    ...
    // because they have CuratedRotationItem rows but zero isFallback sound items.
  ...
  const channelDir = channelSoundCacheDir(cacheRoot, channelId)
