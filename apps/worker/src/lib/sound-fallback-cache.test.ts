...
import { syncChannelSoundFallbackCache } from './sound-fallback-cache.js'

const prismaMock = {
  channel: { findUnique: vi.fn() },
  sound: { findMany: vi.fn() },
...
describe('syncChannelSoundFallbackCache', () => {
  ...
  it('downloads pool files and writes local fallback.m3u', async () => {
    const summary = await syncChannelSoundFallbackCache(prismaMock as never, channelId, root, 24)
  ...
  it('respects maxItems when building the cache pool', async () => {
    ...
    await syncChannelSoundFallbackCache(prismaMock as never, channelId, root, 1)
  ...
  it('writes m3u entries pointing at local cache paths', async () => {
    await syncChannelSoundFallbackCache(prismaMock as never, channelId, root, 24)
  ...
  it('returns zero counts when channel is missing', async () => {
    ...
    const summary = await syncChannelSoundFallbackCache(prismaMock as never, 'missing', root, 24)
