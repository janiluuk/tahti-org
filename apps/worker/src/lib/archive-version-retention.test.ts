...
import { ARCHIVE_REVISION_RETENTION, pruneArchiveRevisions } from './archive-version-retention.js'

vi.mock('./minio.js', () => ({ deleteObject: vi.fn().mockResolvedValue(undefined) }))

describe('pruneArchiveRevisions', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('keeps the original and ten newest revisions', async () => {
    const revisions = Array.from({ length: ARCHIVE_REVISION_RETENTION + 2 }, (_, index) => ({
      id: `version-${index + 2}`,
      rawKey: `raw-${index + 2}`,
      mp3Key: `mp3-${index + 2}`,
      flacKey: null,
    ...
    await pruneArchiveRevisions(prisma, 'archive-1')
  ...
  it('never deletes an active older revision', async () => {
    const revisions = Array.from({ length: ARCHIVE_REVISION_RETENTION + 1 }, (_, index) => ({
      id: `version-${index}`,
      rawKey: `raw-${index}`,
      mp3Key: null,
      flacKey: null,
      isActive: index === ARCHIVE_REVISION_RETENTION,
    }))
    const remove = vi.fn().mockResolvedValue(undefined)
    const prisma = {
      archiveItemVersion: {
